...
class tru_Marketplaces {
  ...
  public function getArticleStatus($OXID = '') {
    ...
    if ( $OXID != '' ) $sSqlArticleStatus .= "WHERE `OXID` = '{$OXID}'";
    $sSqlArticleStatus .= ";";
  ...
  private function getAktiveArticleRecurs($Parent = '', $Level = 1, $Status = 2) {
    ...
    $sSqlAktiveArticle = "SELECT DISTINCT * FROM `{$this->Tbl_Article_Status}` WHERE `PARENTID` = '{$Parent}'";
  ...
  public function getAttributes() {
    if (empty($this->aAttributes)) {
      ...
      $sql = "SELECT `OXOBJECTID`, `TYPE`, `VALUE` FROM `{$this->Tbl_Article_Attributes}` WHERE `OXSHOPID` = '1' AND `OXLOCATION` = 'de';";
  ...
  public function BuildChild() {
    ...
    ********************************************/
    ...
    WHERE tru_channel_article_status.STATUS=2";
    $sSqlChild .= ");";
  ...
  public function Convert2Csv() {
    $this->CsvOutput = $this->tools->array_to_csv($this->aAllProducts, true, "\t", "\n", '');
  }
  ...
  public function convertOutput() {
    ...
    $this->CsvOutput = mb_convert_encoding( $this->CsvOutput, $this->OutputCharset, $GivenCharset);
  }
  ...
  public function editAttribute($aValues) {
    ...
    if ( !empty($aValues) AND is_array($aValues) ) {
      switch ( $aValues['Subjob'] ) {
        case 'Einfuegen':
          $sSqlRaw = "INSERT INTO `%s` ( `OXSHOPID`, `OXLOCATION`, `OXOBJECTID`, `TYPE`, `VALUE` ) VALUES ( '%s',  '%s',  '%s',  '%s',  '%s' );";
          $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Attributes, $aValues['OXSHOPID'], $aValues['OXLOCATION'], $aValues['OXID'], $aValues['Key'], $aValues['Value']);
          break;
        case 'Loeschen':
          $sSqlRaw = "DELETE FROM `%s` WHERE `OXSHOPID` = '%s' AND `OXLOCATION` = '%s' AND `OXOBJECTID` = '%s' AND  `TYPE` = '%s';";
          $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Attributes, $aValues['OXSHOPID'], $aValues['OXLOCATION'], $aValues['OXID'], $aValues['TYPE']);
          break;
        case 'Speichern':
          $sSqlRaw = "UPDATE `%s` SET `VALUE` = '%s' WHERE `OXSHOPID` = '%s' AND `OXLOCATION` = '%s' AND `OXOBJECTID` = '%s' AND `TYPE` = '%s';";
          $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Attributes, $aValues['ValueNew'], $aValues['OXSHOPID'], $aValues['OXLOCATION'], $aValues['OXID'], $aValues['TYPE']);
          break;
        default:
          echo 'Autsch';
      }
  ...
  public function editStatus($aValues) {
    ...
    if ( !empty($aValues) AND is_array($aValues) ) {
      ...
      switch ( $aValues['Subjob'] ) {
        case 'StatusSave':
          $sSqlRaw = "UPDATE `%s` SET `STATUS` = '%s', `COPYFROM` = '%s', `ALIAS` = '%s', `BILDTRANSFER` = '%s', `TEXTTRANSFER` = '%s', `BUILDNAME` = '%s'  WHERE `OXID` LIKE '%s';";
          if ($aValues['Vererbung'] == 'true') $aValues['OrgOXID'] = $aValues['OrgOXID'] . '%';
          $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Status, $aValues['STATUS'], $aValues['COPYFROM'], $aValues['ALIAS'], $aValues['BILDTRANSFER'], $aValues['TEXTTRANSFER'], $aValues['BUILDNAME'], $aValues['OrgOXID']);
          break;
        case 'StatusInsertSave':
          $sSqlRaw = "INSERT INTO `%s` ( `OXID`, `PARENTID`, `STATUS`, `COPYFROM`, `ALIAS`, `BILDTRANSFER`, `TEXTTRANSFER`) VALUES ( '%s',  '%s',  '%s',  '%s',  '%s',  '%s',  '%s' );";
          $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Status, $aValues['OXID'], $aValues['PARENTID'], $aValues['STATUS'], $aValues['COPYFROM'], $aValues['ALIAS'], $aValues['BILDTRANSFER'], $aValues['TEXTTRANSFER']);
          break;
        case 'StatusInsertSaveFormOxid':
          ...
          foreach ( $this->getArticleIdsFromOxid($aValues['PARENTID']) AS $ArticleGr ) {
            $sSqlRaw = "INSERT INTO `%s` ( `OXID`, `PARENTID`, `STATUS`, `BILDTRANSFER`, `TEXTTRANSFER`) VALUES ( '%s',  '%s',  '%s',  '%s',  '%s' );\n";
            $sSql = sprintf($sSqlRaw, $this->Tbl_Article_Status, $ArticleGr['OXID'], $ArticleGr['OXPARENTID'], $aValues['STATUS'], $aValues['BILDTRANSFER'], $aValues['TEXTTRANSFER']);
